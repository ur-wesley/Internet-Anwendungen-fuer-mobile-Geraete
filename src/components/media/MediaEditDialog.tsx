...
const DEFAULT_LOCATION = { lat: 52.5244, lng: 13.4105 };
...
export function MediaEditDialog(props: MediaEditDialogProps) {
  ...
  const [storageType, setStorageType] = createSignal<"local" | "remote">("local");
  const [location, setLocation] = createSignal<{ lat: number; lng: number } | undefined>(undefined);
  ...
  onMount(() => {
    if (navigator.geolocation && props.mode === "create") {
  ...
  async function uploadToRemoteServer(file: File): Promise<string> {
    const formData = new FormData();
    formData.append("image", file, file.name);
    const response = await fetch("https://img.wesley.fyi/upload", {
      method: "POST",
      body: formData,
      headers: {
        "X-API-Key": "ashkglfasfhAKShdsdkSdklad##+a",
      },
    });
    if (!response.ok) {
  ...
  const handleSave = async () => {
    ...
    setError("");
    props.onSave(trimmedTitle, imageSrc, storageType(), location());
  };
        ...
        >
          <div class="flex flex-col gap-2 p-2">
            ...
            <TextFieldRoot class="flex flex-col gap-2">
              ...
              <TextField type="file" accept="image/*" onInput={handleFileChange} class="w-full" />
            </TextFieldRoot>
            ...
            <Show when={imagePreview()}>
              <div class="flex justify-center items-center gap-2">
                <img src={imagePreview()} alt="Vorschau" class="max-h-32 rounded shadow" />
              </div>
            </Show>
            <Show when={error()}>
