...
export class MediaModel {
 ...
 private readonly _updateReactiveStore: (
  key: string,
  value: MediaItem[]
 ) => void;
 ...
 private async initStorage(): Promise<void> {
  if (this.storage) {
   ...
   try {
    const existingItems = await this.storage.getItem<SerializedMediaItem[]>(
     "items"
    );
    ...
    if (existingItems?.length) {
     this._items = existingItems.map((item) => {
      ...
      return item.storageType === "remote"
       ? {
          ...baseItem,
 ...
 private async loadItems(): Promise<void> {
  if (this.storage) {
   try {
    const storedItems =
     (await this.storage.getItem<SerializedMediaItem[]>("items")) || [];
