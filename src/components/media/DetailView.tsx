...
export function DetailView(props: DetailViewProps) {
  ...
  return (
    <div
      class={`fixed inset-0 bg-background transition-opacity duration-300 ${isVisible() ? "opacity-100" : "opacity-0"}`}
    >
      <div class="flex flex-col h-full">
        <header class="flex items-center justify-between p-4 border-b-2 border-border bg-card shadow-sm shrink-0">
          ...
          <Button variant="ghost" size="icon" onClick={handleDelete} class="shrink-0 touch-manipulation">
        ...
        <footer class="p-4 border-t-2 border-border bg-card shadow-sm shrink-0">
          <Button variant="ghost" size="icon" onClick={handleBack} class="touch-manipulation">
